...
const ConfirmModal = ({ open, title, message, confirmText = 'Confirm', cancelText = 'Cancel', onConfirm, onCancel, danger = false }) => {
  ...
  return (
    <div onClick={onCancel} style={{
      ...
      <style>{`@media (max-width: 480px) { .confirm-modal { max-width: 90vw !important; padding: 24px 16px !important; } .confirm-actions { flex-direction: column; } .confirm-actions button { width: 100%; text-align: center; } }`}</style>
      <div onClick={e => e.stopPropagation()} className="confirm-modal" style={{
        ...
        <div className="confirm-actions" style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
